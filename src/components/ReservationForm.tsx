...
import { UnitItem } from './FloorMatrix';
...
export default function ReservationForm({ unit, onSuccess }: ReservationFormProps) {
  ...
  return (
    <div className="bg-white p-6 rounded-3xl shadow-lg border border-gray-100 w-full max-w-lg">
      <div className="border-b pb-4 mb-4">
        <span className="text-xs font-bold text-blue-600 uppercase tracking-widest">Detail Unit Terpilih</span>
        <h3 className="text-2xl font-black text-gray-800">Unit {unit.unitNumber} (Lantai {unit.floor})</h3>
        <p className="text-xs text-gray-500">{unit.facing} - Tipe {unit.type}</p>
      </div>

      <div className="space-y-2 mb-6">
        <label className="text-xs font-bold text-gray-700 uppercase">Pilih Paket Sewa:</label>
        <div className="grid grid-cols-2 gap-2">
          {[
            { key: 'DAILY_1', label: '1 Hari' },
            { key: 'DAILY_3', label: '3 Hari (Diskon 10%)' },
            { key: 'WEEKLY_1', label: '1 Minggu (Diskon 20%)' },
            { key: 'MONTHLY_1', label: '1 Bulan (Rekomendasi)' },
          ].map((item) => (
            <button
              key={item.key}
        ...
        >
          <div>
            <div className="text-xs text-emerald-600 font-bold">1 Tahun (Diskon 50%)</div>
      ...
      <div className="mb-6">
        <label className="text-xs font-bold text-gray-700 uppercase block mb-2">Pilih Payment Gateway:</label>
      ...
      >
        {loading ? 'Memproses Reservasi & Gateway...' : 'Proses Reservasi & Bayar'}
      </button>
    </div>
  );
}